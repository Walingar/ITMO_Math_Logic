...
let get_assumptions_from_header header = 
	...
	let assumptions_tree_list  = parse_assumptions_from_header_to_tree_list (header) in
	List.iteri (fun i tree -> Tree_table.add assumptions_hash_table tree (i + 1)) assumptions_tree_list;
	assumptions_hash_table;;
